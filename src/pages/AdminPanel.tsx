...
import { Integrante, Evento } from '../data/mockData';
...
const ModuloDashboard = ({ onNavigate }: { onNavigate: (modulo: string) => void }) => {
    ...
    const stats = [
        { etiqueta: 'Integrantes', valor: integrantes.length, color: 'text-rose-400', modulo: 'integrantes' },
        { etiqueta: 'Partituras', valor: partituras.length, color: 'text-amber-400', modulo: 'partituras' },
        { etiqueta: 'Eventos', valor: eventos.filter(e => e.activo).length, color: 'text-blue-400', modulo: 'eventos' },
        ...
        <div className="space-y-6">
                ...
                {stats.map((s, i) => (
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <ChevronRight className="w-3 h-3 text-white/20 group-hover:text-vinotinto-claro 
            ...
            >
                <div className="flex items-center justify-between mb-3">
                    ...
                    <ChevronRight className="w-4 h-4 text-white/20 group-hover:text-vinotinto-claro 
                        group-hover:translate-x-0.5 transition-all" />
                </div>
                <div className="flex items-center gap-3">
                    <p className="text-3xl font-display font-bold text-blue-400">{mensajesNoLeidos}</p>
                ...
                {configuracionSecciones.tipoAsistente === 'whatsapp' && (
                    <div className="mt-4 p-4 rounded-lg bg-green-500/10 border border-green-500/20 space-y-3">
                        <label className="block">
                            <span className="text-xs text-green-400 font-medium">Número de WhatsApp</span>
                            <div className="flex gap-2 mt-1">
                                ...
                                <input
                                    ...
                                    className="flex-1 bg-sutil border borde-subtle rounded-lg px-3 py-2 text-sm text-secundario placeholder-white/30 focus:outline-none focus:border-green-500/50 transition-colors font-mono"
                    ...
                    <div key={e.id} className="card-glass rounded-xl p-4 flex items-start gap-4">
                        <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className={`text-xs px-2 py-0.5 rounded-full ${e.activo ? 'bg-emerald-500/20 text-emerald-400' : 'bg-sutil-hover t-muted-low'}`}>
...
// ============================================================
const AdminPanel = () => {
    const { esAdmin, usuarioActual, cerrarSesion, solicitudesAudicion, mensajesContacto } = useApp();
    ...
    const COMPONENTES_MODULOS: Record<string, JSX.Element> = {
        ...
        'partituras': <div className="t-muted text-center py-20"><BookOpen className="w-12 h-12 mx-auto mb-3 opacity-30" /><p>Gestión de partituras: añade/edita/elimina en el código (mockData.ts)</p><p className="text-xs mt-2">Interfaz completa disponible en Fase 2 con Supabase Storage para PDFs</p></div>,
        'audio': <div className="t-muted text-center py-20"><Music className="w-12 h-12 mx-auto mb-3 opacity-30" /><p>Gestión de pistas de audio: edita pistasAudioDefault en mockData.ts</p><p className="text-xs mt-2">Subida de audio desde la interfaz disponible en Fase 2</p></div>,
                            ...
                            >
                                ...
                                {badge > 0 && (
                                    <span className="flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-full 
                                        bg-amber-500/20 text-amber-400 text-[10px] font-bold border border-amber-500/30
                                        animate-pulse-soft">
                                        {badge}
                                    </span>
                                )}
