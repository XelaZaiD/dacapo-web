...
import { Menu, X, Moon, Sun, Music, LogIn, LogOut, Shield, User } from 'lucide-react';
...
const Navbar = () => {
    ...
    const { estaLogueado, esAdmin, usuarioActual, cerrarSesion, modoOscuro, toggleModoOscuro, configuracionSecciones } = useApp();

    // Estado local: si el menú móvil está abierto
    const [menuMovilAbierto, setMenuMovilAbierto] = useState(false);
    // Estado local: si el modal de autenticación está abierto
    ...
    const location = useLocation(); // Nos dice en qué ruta estamos
        ...
        <>
            ...
            <motion.nav
                className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${navConFondo
                        ? 'bg-fondo-oscuro/95 backdrop-blur-md border-b border-white/10 shadow-lg'
                        : 'bg-transparent'
                    }`}
            ...
            >
                <div className="contenedor">
                    <div className="flex items-center justify-between h-16 md:h-20">
                        ...
                        <Link to="/" className="group flex items-center gap-2">
                            {/* Ícono musical */}
                            <div className="w-9 h-9 bg-vinotinto rounded-lg flex items-center justify-center
                              group-hover:bg-vinotinto-claro transition-colors duration-300">
                                <Music className="w-5 h-5 text-white" />
                            </div>
                            {/* Nombre del grupo */}
                            <div className="flex flex-col leading-none">
                                <span className="font-display font-bold text-xl text-secundario tracking-tight">
                                    DaCapo
                                </span>
                                <span className="text-[10px] text-khaki uppercase tracking-[0.2em] font-semibold">
                                    Grupo Vocal
                                </span>
                            </div>
                        </Link>
                        ...
                        <nav className="hidden md:flex items-center gap-1">
                            ...
                            {itemsFiltrados.map(item => (
                                <button
                                    ...
                                    className="px-3 py-2 text-sm font-medium text-white/70 hover:text-khaki
                        ...
                        <div className="flex items-center gap-2">
                           ...
                           text-white/60 hover:text-khaki hover:bg-white/5
                           ...
                           text-white/70 hover:text-khaki hover:bg-white/5 transition-all"
                            >
                ...
                <AnimatePresence>
                    {menuMovilAbierto && (
                        <motion.div
                            className="md:hidden bg-fondo-oscuro/98 backdrop-blur-xl border-t border-white/10"
                            ...
                            <div className="contenedor py-4 flex flex-col gap-1">
                                {itemsFiltrados.map(item => (
                                    <button
                                        key={item.href}
                                        onClick={() => hacerScrollA(item.href)}
                                        className="text-left px-4 py-3 text-white/70 hover:text-khaki
                               hover:bg-white/5 rounded-lg transition-all duration-200"
                                    >
                                        {item.etiqueta}
                                    </button>
                                ...
                                {!estaLogueado && (
                                    <button
                                        onClick={() => { setModalAuthAbierto(true); setMenuMovilAbierto(false); }}
